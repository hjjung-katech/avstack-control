...
4. **벤더 트랙**: 문의 2건(MORAI-001 AVS-007 / MORAI-002 AVS-006) **발송 준비 완료 — 재검증 3회(2026-07-06)·§3 N 기입 완료**. **사용자 액션 1순위** = ① 발송(제목 "내부 ID..." + INTERNAL 블록 제거) → ② 발송 직후 별도 세션에서 SENT 동결·vendor_comms SENT 전이 행. MORAI-003(기능요청) 초안 대기. (vendor/ 체계 신설 완료)
...
| 문서·벤더 관리 규약 | `runbooks/doc_and_vendor_management.md` | 문서체계·벤더 커뮤니케이션 규약 | v1.1 |
| MORAI 문의 (OUTBOX) | `vendor/morai/OUTBOX/MORAI-001,002_*.md` | AVS-007/006 문의 패키지 | **발송 대기 (N회 기입 후 사용자 발송)** |
...
|---|---|---|---|---|
| MORAI-001 | AVS-007 | ROS2 Native ros2cs 예외 + rosbridge header.seq 실측 보고·질의 | **DRAFT** | **재검증 완료(N=3, 2026-07-06) — 발송 준비 완료** |
| MORAI-002 | AVS-006 | sourcedefender 보호 API의 지원 Python·설치·라이선스 질의 | **DRAFT** | **재검증 완료(N=3, 2026-07-06) — 발송 준비 완료** |
| MORAI-003 | R18 (기능요청) | ROS 연결 CLI/설정파일화·자동 Connect·런처 자동기동 인자·Headless; sourcedefender 제거·공식 아카이브(제품개선) | TODO_DRAFT | 초안 작성(001 회신 흐름에 병행) |

## 7. 다음 세션 착수 순서 (우선순위)

1. **MORAI-001/002 발송 — 최고 레버리지 사용자 액션 (1순위)**. §3 N=3 기입 완료. 순서: ① 발송(복사 시 제목의 "(내부 ID...)" 제거 + `<!-- INTERNAL -->` 블록 전부 제거) → ② **발송 직후 별도 세션**에서 SENT 동결(발송본을 `vendor/morai/SENT/YYYYMMDD_*.md`로) + vendor_comms.tsv SENT 전이 행 append. (②는 발송 전 착수 금지.)
