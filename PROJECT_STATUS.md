...
2. **SIM-불필요 트랙** — 로그 스키마 인벤토리 **완료(2026-07-06, 기존 로그 3회분)**. 다음 = parser 골격·golden fixture([EVAL] avstack-eval 생성 전제); 시나리오 다양화·실 PASS/FAIL 케이스는 SIM 실행(GUI) 필요.
3. (PC 접근 가능 시) **D1~D5 진단** — 사람 1분 세팅 후 원격 완주, AVS-007 귀속 확정.
4. **[EVAL] 리포 착수 검토** — log_schema_inventory 기반 parser 골격·golden fixture (avstack-eval 생성 시점). (ADR-009 등재·erratum 5.8/9.4 = 2026-07-06 완료)
