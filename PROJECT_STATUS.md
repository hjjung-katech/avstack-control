...
- 준비-스테이징(자율): **morai_ros2_msgs 소스 스테이징 완료 (2026-07-28)** — `26.R1` 브랜치 clone, HEAD=`c84d648`(핀 일치), 32 msg/6 srv(`CtrlCmd`·`EgoVehicleStatus` 포함). 위치 `~/avstack/ros2_ws_26r1/src/morai_ros2_msgs`(WS non-git). **빌드는 ROS2 humble 설치 후**(=다음 sudo 게이트). 증거 `nw_msgs_stage_20260728.txt`.
