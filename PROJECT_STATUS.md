...
- 준비-스테이징(자율): **msgs 소스 2종 스테이징 완료 (2026-07-28)** — ① `morai_ros2_msgs` git `26.R1`/`c84d648`(핀 일치), ② `morai_msgs` 랩탑 벤더본 Restore(non-git). EgoVehicleStatus 등 내용 동일(패키지명만 다름). 32 msg/6 srv. 위치 `~/avstack/ros2_ws_26r1/src/`. **빌드는 ROS2 humble 설치 후**; SIM 발행 타입(morai_msgs vs morai_ros2_msgs)은 native 게이트 `ros2 topic type`로 확정 후 매칭 빌드. 증거 `nw_msgs_stage_20260728.txt`.
