...
4. **벤더 트랙**: 문의 2건(MORAI-001 AVS-007 / MORAI-002 AVS-006) **발송 준비 완료** — 사용자 액션 = §3 N회 기입 후 발송. MORAI-003(기능요청) 초안 대기. (vendor/ 체계 신설 완료)
...
| 완전 자율화 분석 | `runbooks/full_autonomy_analysis.md` | GUI 자동화 vs 아키텍처 우회(E안) | **(배치 대기 — 사용자 업로드 필요)** |
...
1. **MORAI-001/002 발송 (N회 기입 후)** — 최고 레버리지 사용자 액션. 발송 즉시 vendor_comms.tsv에 SENT 행 append + SENT/ 동결(INTERNAL 제거본).
