...
- ROS2 Native 토픽 게이트: **진행중** — 1차 시도서 ROS2 Connect 순간 SIM 크래시(SIGSEGV). **근본원인=fastdds ABI 불일치**: SIM ros2cs는 fastdds **2.6.4** 빌드인데 호스트 humble=**2.6.11** → `DomainParticipantFactory` 세그폴트(원본 Player.log 스택 확인). **NW-01 정정: `ros2-fastdds-snap`(2.6.4, FASTDDS_PREFIX용)은 Restore 자산인데 초기 이관에서 누락 → Restore 완료(19M).** wrapper가 이제 overlay(ros2_ws_26r1)+fastdds snap 자동 감지(AVS-007 #2 footgun 제거). SIM 발행 타입=**morai_ros2_msgs**(t24 §6 N3). 재시도 명령: `SOURCE_ROS2=1 ./scripts/run_morai_launcher_nvidia.sh` → Start→ROS2 Connect→설정창 닫기 → Claude가 50Hz/CtrlCmd 검증.
