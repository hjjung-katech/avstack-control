...
- ROS2 Native 토픽 게이트: **진행중** — 1차 시도서 ROS2 Connect 순간 SIM 크래시(SIGSEGV). **근본원인=fastdds ABI 불일치**: SIM ros2cs는 fastdds **2.6.4** 빌드인데 호스트 humble=**2.6.11** → `DomainParticipantFactory` 세그폴트(원본 Player.log 스택 확인). **NW-01 정정: `ros2-fastdds-snap`(2.6.4, FASTDDS_PREFIX용)은 Restore 자산인데 초기 이관에서 누락 → Restore 완료(19M).** 2차 시도도 동일 크래시 → 원인=**snap prefix 미적용**(FASTDDS_PREFIX가 `SOURCE_ROS2=1` 게이트 안, 사용자가 미설정 시 스킵 + bashrc가 humble 2.6.11 소싱). **정정: snap은 fastrtps만 10개(librcl/librmw 없음)이라 humble+snap 둘 다 필요 → wrapper `SOURCE_ROS2` 기본값 ON으로 변경**, launch env 덤프(.env) 추가. ldd/시뮬로 `libfastrtps.so.2.6→snap 2.6.4` 해결 확인. SIM 발행 타입=**morai_ros2_msgs**(t24 §6 N3). 재시도: `./scripts/run_morai_launcher_nvidia.sh`(env var 불요) → Start→ROS2 Connect→설정창 닫기 → Claude가 .env 확인+50Hz/CtrlCmd 검증.
- **다음 실작업(오후 재개)**: 위 재시도(SIM+ROS2 Connect)를 사용자 GUI로 → Claude 자율 검증. 이후: SR/Built-in, py API(Restore zip→miniconda). 원격=NoMachine `100.70.233.26:4000` 물리 데스크톱.
