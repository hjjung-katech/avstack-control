...
4. **벤더 트랙**: 문의 2건(MORAI-001 AVS-007 / MORAI-002 AVS-006) **발송 준비 완료**. **사용자 액션 1순위** = ① §3 `[N회 시도]` 기입 → ② 발송 → ③ 발송 직후 별도 세션에서 SENT 동결(INTERNAL 제거본 저장)·vendor_comms SENT 전이 행. MORAI-003(기능요청) 초안 대기. (vendor/ 체계 신설 완료)
...
(vendor_comms.tsv comm_id별 최신 행 기준, 2026-07-04)

| Comm ID | 대상 이슈 | 요지 | 상태 | 다음 액션 |
|---|---|---|---|---|
| MORAI-001 | AVS-007 | ROS2 Native ros2cs 예외 + rosbridge header.seq 실측 보고·질의 | **DRAFT** | §1/§3 보강 완료 — N회 기입 후 사용자 발송 |
| MORAI-002 | AVS-006 | sourcedefender 보호 API의 지원 Python·설치·라이선스 질의 | **DRAFT** | 초안 완료 — 사용자 검토·발송 |
...
1. **MORAI-001/002 발송 — 최고 레버리지 사용자 액션 (1순위)**. 순서: ① §3 `[N회 시도]` 기입 → ② 발송 → ③ **발송 직후 별도 세션**에서 SENT 동결(INTERNAL 제거본을 `vendor/morai/SENT/YYYYMMDD_*.md`로) + vendor_comms.tsv SENT 전이 행 append. (③은 발송 전 착수 금지.)
