...
- **다음 실작업(오후 재개)**: **ROS2 Native 토픽 게이트** — 트리거=사용자 GUI(SIM 실행→설정창에서 ROS2 Connect). 그 후 Claude 자율: `ros2 topic type /ego_vehicle_status`로 발행 타입 확정(morai_msgs vs morai_ros2_msgs) → 매칭 오버레이 소싱(`~/avstack/ros2_ws_26r1/install`) → `/ego_vehicle_status` 50Hz 수신 + `CtrlCmd` 제어 실측(**RMW 미설정 유지**, SIM은 `scripts/run_morai_launcher_nvidia.sh`). 이후: SR/Built-in 시나리오, py API(Restore zip→miniconda 필요). 원격은 NoMachine(`100.70.233.26:4000`, 물리 데스크톱)으로.
