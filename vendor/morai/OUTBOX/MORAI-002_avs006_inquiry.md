# MORAI 문의 — OpenSCENARIO API 22.R3 sourcedefender 실행 (내부 ID: MORAI-002 / 관련: AVS-006)

<!-- INTERNAL: 그대로 복사해 MORAI 기술지원에 보낼 수 있는 문의문(한국어). SENT 동결 시 INTERNAL 블록 제거. -->

## 1. 요약 (3줄 이내)
OpenSCENARIO API 22.R3의 lib이 sourcedefender로 암호화(694개 `.pye`, 암호화 시점 Python 3.7 마이너에 고정)돼 있는데,
3.7 호환 sourcedefender 런타임이 PyPI에서 제거되어(현재 8개 릴리스 전부 `>=3.9/3.10`) API import 자체가 불가합니다.
...
```
- PyPI 실태(pypi.org/pypi/sourcedefender/json, 2026-07-03): 릴리스 **8개뿐** — 15.0.19~22(`>=3.9`), 16.0.61~64(`>=3.10`),
  전부 sdist(tar.gz), **cp37 wheel 0개** → 3.7 지원 버전이 PyPI에서 삭제됨. (sourcedefender 개발사 문서(공식 사이트)는 "3.7–3.14 지원" 주장, 실제 배포와 불일치.)
<!-- INTERNAL: 근거 사본(git 동결) vendor/morai/evidence/avs006_sourcedefender_py37_block_20260703.md. SENT 동결 시 이 줄 제거. -->
...
<!-- INTERNAL: 제품 개선 성격의 요청(sourcedefender 제거 정책·공식 아카이브 등)은 MORAI-003으로 분리. SENT 동결 시 이 줄 제거. -->
