...
| `~/avstack-control` git init + 원격 | 완료 | private GitHub `hjjung-katech` (SSH), `main` 동기화 중 |
| CLAUDE.md 확정본 생성 | 완료 | 19.1 확정본과 동일 |
| MORAI Launcher/SIM/SR 배치 | 완료 | `~/avstack/morai/launcher/MoraiLauncher_Lin/`(심볼릭 링크→`_b` 슬롯); SIM·SR은 그 하위에 설치됨 (5.1) |
| Stage 01 MORAI SIM | 완료 (PASS) | Vulkan+NVIDIA 정상; 지도 부분표시는 AVS-002로 추적 |
| Stage 02 Scenario Runner | 완료 (PASS) | `libxcb-xinerama0`+`liblapack3` 설치 후 SIM 경유 동작 (AVS-001 해결) |
| 기록 인프라 | 완료 | `records/*.tsv` 3종 + `record_stage/issue/decision.sh` |

### 0.2 미완료 (다음 작업 순서 — 이 순서대로 진행한다)

| 순서 | 작업 | 절차 위치 |
|---:|---|---|
| 1 | AVS-002 진단 (지도 부분표시: NoMachine Vulkan 전송 vs 4GB VRAM) | 10.4 |
| 2 | Stage 03: 예제 XOSC Built-in 실행 | 12장 |
| 3 | `.claude/settings.json` + `settings.local.json` 생성 | 19.2 |
| 4 | Claude Code 설치 + 첫 세션 검증 + ADR-005 기록 | 18장 |
| 5 | `env/ros2_humble.env`, `.gitignore` 생성 후 Stage 04 (ROS2) | 13장 |

주: v1.2가 상정했던 `bootstrap_avstack_control.sh`는 실제로 만들지 않았고, 기록 인프라(`records/*.tsv` + `record_*.sh`)는 세션에서 직접 생성했다. OPEN-05(계정 ID)로 SR 단독 로그인은 불가하나 SIM 경유 우회가 되므로 blocker 아님.
...
| OPEN-05 | MORAI 계정 ID/권한 | 부분확인 | SR **단독 실행** 로그인에 계정 ID 필요(현재 없음). **SIM 경유로 SR 실행은 가능**하므로 blocker 아님. 온라인 라이선스 인증 방식(동글 아님, keylok 룰 미설치) |
...
| `morai/launcher` | **실제 배치 완료.** `MoraiLauncher_Lin`(심볼릭 링크) → `MoraiLauncher_Lin_a`/`_b` (버전별 A/B 슬롯; 업데이트 시 링크만 교체). 실행 파일 `MoraiLauncher_Lin/MoraiLauncher_Lin.x86_64` (대문자 L) 및 공식 실행 스크립트 `MORAISim.sh` |
| SIM/SR 실제 위치 | Launcher 하위에 설치됨: `MoraiLauncher_Lin/MoraiLauncher_Lin_Data/Project/Simulator/…/Simulator.x86_64`, `MoraiLauncher_Lin/MoraiLauncher_Lin_Data/ScenarioRunner/scenario_runner_v1.7.0_linux/program/Scenario Runner`. `morai/sim`·`morai/scenario_runner`는 사실상 미사용 |
...
### 10.2 실행 방법 (v1.4: 공식 `MORAISim.sh` 반영)

**공식 매뉴얼 방식** (MORAI SIM: Drive User Manual, Installation and Setup — 25장 링크):
...
`MORAISim.sh`는 keylok USB 동글 udev/hotplug 룰과 Kvaser CAN 심볼릭 링크를 준비한 뒤 `MoraiLauncher_Lin.x86_64`를 실행하는 **공식 런처 래퍼**다. 매뉴얼은 GPU offload를 언급하지 않으므로, 우리는 이 래퍼를 **NVIDIA PRIME offload로 감싸서** 실행한다.

**우리 스크립트:** `~/avstack-control/scripts/run_morai_launcher_nvidia.sh` (전문은 배치된 파일이 원본). 핵심:

- NVIDIA offload 환경변수 export 후 `MoraiLauncher_Lin/MORAISim.sh` 실행. **SIM은 Vulkan으로 렌더**하므로(로그 확인) 실효 변수는 `__NV_PRIME_RENDER_OFFLOAD`·`__VK_LAYER_NV_optimus`이고 `__GLX_VENDOR_LIBRARY_NAME`은 no-op(무해).
- 경로는 심볼릭 링크 `MoraiLauncher_Lin` 기준 → A/B 슬롯 업데이트에 안전.
- 원격(SSH→NoMachine)에서 띄울 땐 `RUN_REMOTE=1`로 `DISPLAY=:1`, `XAUTHORITY=~/.Xauthority`를 붙인다 (18.6).
...
### 10.4 통과 기준 및 결과 (Stage 01 PASS)

기준: Launcher 창 표시 / SIM 설치·실행 / 기본 지도 로드 / NVIDIA 사용 / 정상 종료.
...
> **⚠️ GPU 최소사양:** 매뉴얼 최소사양은 **RTX 2060 Super**인데 이 머신은 **RTX 3050 Laptop 4GB로 미달**이다. 지도가 NoMachine 화면에 **일부만 표시**되는 증상(AVS-002)은 (a) NoMachine의 Vulkan swapchain 전송 불완전, 또는 (b) 4GB VRAM 텍스처 스트리밍 하향이 원인으로 추정된다. SIM 내부 로그엔 렌더 실패가 없어 **표시 계층 문제**로 본다. 진단: 콘솔 직접 표시 vs NoMachine 비교, 지도 로드 중 `nvidia-smi` VRAM 관찰, 경량 맵/품질 시도.
...
### 11.1 AVS-001 (RESOLVED, Stage 02 PASS)

| 항목 | 내용 |
|---|---|
| ID | AVS-001 (**RESOLVED**, HIGH) |
| Symptom | Start Scenario Runner 버튼은 활성화되나 클릭 후 창이 뜨지 않음 |

**근본 원인 (확정):** Scenario Runner는 **PyInstaller + PySide2(Qt 5.15) GUI 앱**(`…/ScenarioRunner/scenario_runner_v1.7.0_linux/program/Scenario Runner`, `_internal/` 번들). 실행 중 native 라이브러리 2개가 없어 **UI 생성 전에 종료**했고, Launcher가 자식 프로세스 stderr를 삼켜 아무 메시지 없이 "버튼만 활성"으로 보였다.

1. `libxcb-xinerama0` 누락 → Qt xcb 플랫폼 플러그인(`libqxcb.so`) 로드 실패
2. `liblapack3`/`libblas3` 누락 → numpy import 단계 `liblapack.so.3` ImportError

**배제된 가설:** "Launcher가 Intel OpenGL 상속"(가설 1) — SIM/SR 경로 모두 Vulkan+NVIDIA로 정상. 실제는 가설 2(dependency 누락) + 가설 3(자식 stderr 은폐)의 결합.

**해결:** 두 패키지 설치(별도 SSH/NoMachine 터미널에서 `sudo apt install -y libxcb-xinerama0 liblapack3`) 후, **MORAI SIM을 먼저 열고 그 안에서 Scenario Runner를 실행**하면 메인 UI까지 정상 진입한다. (SR **단독 실행**은 로그인에 계정 ID가 필요 — OPEN-05.)

> 진단 요령: SR 바이너리를 터미널에서 직접 실행하면(Launcher 우회) stderr에 원인이 찍힌다. `ldd "…/libqxcb.so" | grep "not found"`로 누락 라이브러리를 정적 확인, `QT_DEBUG_PLUGINS=1`로 플러그인 로드 과정을 본다. 아래 11.2/11.3은 이때 쓴 진단 절차다.
...
### 18.6 원격 GUI 운영법 (v1.4 신설: SSH + NoMachine)

이 머신은 노트북 한 대이며 두 채널로 접근한다: **SSH(터미널, 화면 없음)** 와 **NoMachine(원격 데스크톱, `DISPLAY=:1`)**. Claude Code는 SSH 채널에서 실행되므로 기본적으로 화면이 없다.

핵심: **SSH 세션에서 `DISPLAY=:1`로 GUI 앱을 실행하면 그 창이 NoMachine 화면에 나타난다.** 접근 확인:

```bash
DISPLAY=:1 XAUTHORITY="$HOME/.Xauthority" glxinfo -B >/dev/null && echo OK
```

GUI 앱 실행 형태 (화면 채널 + NVIDIA offload):

```bash
DISPLAY=:1 XAUTHORITY="$HOME/.Xauthority" \
  __NV_PRIME_RENDER_OFFLOAD=1 __VK_LAYER_NV_optimus=NVIDIA_only "<앱>"
```

역할 분담: CLI(설치·의존성·로그·기록)와 **GUI 앱 실행**은 SSH의 Claude가 처리하고, **창이 실제로 떴는지 확인**은 사용자가 NoMachine 화면에서 한다. `run_morai_launcher_nvidia.sh`는 `RUN_REMOTE=1`로 이 변수를 자동 설정한다.

> sudo 설치는 `!` 비대화형 채널에서 안 된다(비밀번호 프롬프트 불가). 별도 SSH/NoMachine **실제 터미널**에서 직접 실행한다.
...
실제 생성본(v1.4): `record_stage.sh`(6열: date stage status summary evidence next), `record_issue.sh`(10열: id date stage status severity symptom hypothesis evidence next resolution), `record_decision.sh`(5열: id date status decision reason), `run_morai_launcher_nvidia.sh`(10.2). 각 record 스크립트는 헤더가 없으면 TSV를 자동 생성하고, 자유 텍스트의 탭/개행을 제거해 TSV를 보호한다. **TSV 직접 편집 금지 — 이 스크립트로만 기록.** (v1.2가 상정한 `bootstrap_avstack_control.sh`·`capture_env_snapshot.sh`는 미생성.)
...
- **MORAI SIM: Drive User Manual — Installation and Setup** (공식 실행법 `MORAISim.sh`, GPU 최소사양 RTX 2060 Super, Vulkan `libvulkan1`, 온라인 라이선스): https://morai-sim--drive-user-manual--en-22-r2.scrollhelp.site/msdume2/installation-and-setup
...
| MORAI 실행 | `run_morai_launcher_nvidia.sh` → 공식 `MORAISim.sh` 래퍼 + NVIDIA Vulkan offload (10.2) |
| Docker | 보류 (Stage 05 이후) |
| Claude Code | `~/avstack-control` 전용, settings.local.json으로 logs/runs 접근 |
| 현재 Stage | 00~02 PASS → 다음 Stage 03 (예제 XOSC) |
| 주요 이슈 | AVS-001 RESOLVED / AVS-002 지도 부분표시 (OPEN, MED) |
