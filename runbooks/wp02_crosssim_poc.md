...
- **매트릭스·수용기준 정의의 정본 = `mgmt` 의 plans/WP-02** (외부 저장소, 계층 ④). 본 문서에서 매트릭스를
  재정의·복제하지 않는다.
- **본 문서(계층 ③)의 역할 = 실행 절차 + 실행 기록·증거 위치**. 판정은 `records/stages.tsv` 단일 권위.
...
## 2. 선행 조건 *(착수 시 확정)*
- [ ] 대상 시뮬레이터/버전, 공통 시나리오 소스(oss3-scenarios, 계층 ①)
- [ ] 실행 환경(계층 ②, ~/avstack) 준비
...
- 원본 로그/런 산출물: `~/avstack/runs`, `~/avstack/logs` (non-git, 계층 ②) — TSV에는 경로만.
